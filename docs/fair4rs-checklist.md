...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/weatherxbiodiversity-lizards`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/weatherxbiodiversity-lizards:latest` over HTTPS. |
...
- [ ] If this is the first release: `CITATION.cff` `doi` field is left as `10.5281/zenodo.20266646` (the integration mints it on release; update afterwards).
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/weatherxbiodiversity-lizards:latest`.
