...
Figures: figures/main_result.png (sensitivity-matrix heatmap, headline). figures/per_species_rates_baseline.png (baseline-config per-species lollipop, all species at 0 % — uniform-across-species proof-point). figures/per_species_rates_s3a.png (S3a-config per-species lollipop, showing which species the mechanism flags first under the worst-plausible prior; Podarcis virescens, Psammodromus occidentalis, P. vaucheri, Acanthodactylus erythrurus, Podarcis carbonelli are the top five well-sampled species in the 2-5 % range). figures/substrate_sensitivity.png (Bombus-chain-3-pattern scatter, nside=128 vs nside=64 per species, S3a config). figures/h_r_map_2020s.png, figures/h_r_map_2030s.png (Iberia maps of decadal-mean daily-mean h_r rendered as HEALPix nside=128 NESTED tiles via the EOPF-DGGS healpix-plot library per DOMAIN.md canonical pattern).
```
