...
(5) Daily Tmax vs hourly trace. SOM Eq. S2 is parameterised in daily Tmax and computed daily; the analysis reported here applies the same form. A sub-daily Polytope-access probe in the data-download step of the cited Research Software returned "credentials absent — fallback unused"; sinusoidal diurnal-cycle reconstruction was scoped but not needed because the mechanism is daily-Tmax-driven, not hourly-integration-driven. The probe outcome is recorded in the Research Software deposit's results-registry artefact.
```
