...
Substrate-sensitivity diagnostic (mirrors Bombus chain-3 pattern, S3a config): Spearman ρ on per-species 2020s local-extinction rates between HEALPix nside=128 (DestinE native) and HEALPix nside=64 (downsampled via NESTED bit-shift parent = pix >> 2). Full set: ρ = 0.891 (n=47). Well-sampled (n_cells ≥ 10): ρ = 0.951 (n=25). Low-N (n_cells < 10): n/a (n=22; zero variance at both substrates because all 22 species' rates collapse to 0 % at this signal level). Per-species substrate comparison table: results/tables/substrate_sensitivity_per_species.csv.

Figures: figures/main_result.png (sensitivity-matrix heatmap, headline — makes the prior-conditional behaviour visible by showing all six T_b × window combinations). figures/per_species_rates.png (per-species lollipop under the S3a config; well-sampled species topping the 2020s ranking: Podarcis virescens (n=63, ~5 %), Psammodromus occidentalis (n=49), Podarcis vaucheri (n=41), Acanthodactylus erythrurus (n=145), Podarcis carbonelli (n=10) — the baseline-config variant of this figure is omitted as redundant since all species sit at 0 % which is already conveyed by the sensitivity heatmap). figures/substrate_sensitivity.png (Bombus-chain-3-pattern scatter, nside=128 vs nside=64 per species, S3a config). figures/h_r_map_2020s.png, figures/h_r_map_2030s.png (Iberia maps of decadal-mean daily-mean h_r rendered as HEALPix nside=128 NESTED tiles via the EOPF-DGGS healpix-plot library per DOMAIN.md canonical pattern).
```
